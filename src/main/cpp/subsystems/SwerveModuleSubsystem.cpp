...
SwerveModuleSubsystem::SwerveModuleSubsystem(int m_MotorController, rev::SparkMaxRelativeEncoder::Type m_EncoderType, int m_counts_per_rev, 
  ...
  //Can't independently invert drive encoder from drive motor.

  // Set the distance per pulse for the drive encoder. We can simply use the
  // distance traveled for one rotation of the wheel divided by the encoder
  // resolution.
  m_driveEncoder->SetPositionConversionFactor(ModuleConstants::kDriveEncoderDistancePerPulse);
  m_driveEncoder->SetVelocityConversionFactor(ModuleConstants::kDriveEncoderDistancePerPulse / 60.0); //Converting RPM to Meters per second
  ...
  m_turningEncoder->ConfigFeedbackCoefficient(0.001534,"Radians", ctre::phoenix::sensors::SensorTimeBase());
  ...
  m_drivePIDController.SetP(frc::SmartDashboard::PutNumber("Enter P Value for Drive" + std::to_string(m_driveMotor->GetDeviceId()), ModuleConstants::kPModuleDriveController));
  m_turningPIDController.SetP(frc::SmartDashboard::PutNumber("Enter P Value for Turn" + std::to_string(m_turningMotor->GetDeviceId()), ModuleConstants::kPModuleTurningController));
...
void SwerveModuleSubsystem::SetDesiredState(const frc::SwerveModuleState& referenceState) {
  ...
  auto driveOutput = m_drivePIDController.Calculate(m_driveEncoder->GetVelocity(), referenceState.speed.to<double>());
  ...
  m_turningPIDController.SetP(frc::SmartDashboard::GetNumber("Enter P Value for Turn" + std::to_string(m_turningMotor->GetDeviceId()), 1E-5));
  
  auto turnOutput = m_turningPIDController.Calculate(units::radian_t( m_turningEncoder->GetPosition() /* * 78.73*/ + m_wheelOffset), referenceState.angle.Radians());
